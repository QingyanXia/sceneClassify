...
def main():
    ...
    # 模型：SEResNet，但分类层去掉 Dropout（单层 Linear）
    model = SEResNet(num_classes=6, reduction=16).to(device)
    model.fc = nn.Linear(512, 6)   # 移除 Dropout
    ...
    torch.save(model.state_dict(), 'checkpoints/ablate_no_dropout_best.pth')
    train_time = time.time() - start_time
    print(f"训练完成，耗时: {train_time:.2f}秒")

    # 测试
    print("在测试集上评估...")
    from utils.train_eval import test_model
    test_acc, cm, _, _ = test_model(model, test_loader, device)
    print(f"测试准确率: {test_acc:.4f}")

    # 保存结果
    np.savez('experiments/ablate_no_dropout_results.npz',
             history=history,
             test_acc=test_acc,
             cm=cm,
