...
# 加载阶段十一结果
data = np.load('experiments/stage11_80epoch_results.npz', allow_pickle=True)
...
axes[0].set_title('Stage11: Training and Validation Accuracy')
axes[0].legend()
axes[0].grid(True)
...
axes[1].set_title('Stage11: Training and Validation Loss')
axes[1].legend()
axes[1].grid(True)
...
axes[2].set_title(f'Stage11: Confusion Matrix (Test Acc: {test_acc:.2%})')

plt.tight_layout()
plt.savefig('experiments/stage11_combined.png', dpi=150)
print("图像已保存至 experiments/stage11_combined.png")
plt.show()