...
data = np.load('experiments/stage8_weighted_results.npz', allow_pickle=True)
...
axes[0].set_title('Stage8: Training and Validation Accuracy')
axes[0].legend()
axes[0].grid(True)
...
axes[1].set_title('Stage8: Training and Validation Loss')
axes[1].legend()
axes[1].grid(True)

disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class_names)
disp.plot(ax=axes[2], cmap='Blues', xticks_rotation='vertical', colorbar=False)
axes[2].set_title(f'Stage8: Confusion Matrix (Test Acc: {test_acc:.2%})')

plt.tight_layout()
plt.savefig('experiments/stage8_combined.png', dpi=150)
plt.show()